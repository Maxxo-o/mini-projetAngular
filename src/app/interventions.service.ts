...
export class InterventionsService {
  ...
  getAllInterventions(): Observable<Intervention[]> {
    return this.http.get<Intervention[]>('http://localhost:4200/Intervention');
  }
  getInterventionById(id: number): Observable<Intervention> {
    const intervention= this.http.get<Intervention>('http://localhost:4200/Intervention/'+ id);
  ...
  newAddIntervention(intervention: Intervention): Observable<Intervention> {
    return this.getAllInterventions().pipe(
      ...
      switchMap(()=> this.http.post<Intervention>('http://localhost:4200/Intervention', intervention))
      );
  }
}